...
class UserDB:
    ...
    def get_or_create_user(self, user_id: int, username: str = None, first_name: str = None,
        ...
        if not user:
            ...
            if referral_code:
                cursor.execute(
                    "SELECT referrer_id, referrer_username, commission_percent, bonus_months "
                ...
                if link_info:
                    ...
                    if referrer_id and referrer_id < 0 and referrer_username:
                        cursor.execute(
                            ...
                            (referrer_username.lower(),)
                        )
                        real_user = cursor.fetchone()
                        if real_user:
                            referrer_id = real_user[0]
                            cursor.execute(
                                "UPDATE referral_links SET referrer_id = ? WHERE code = ?",
    ...
    def get_user_by_short_id(self, short_id: int) -> Optional[Dict]:
        """Получает пользователя по внутреннему короткому ID"""
        cursor = self.conn.cursor()
        cursor.execute('''
    ...
    def get_all_users(self) -> List[Dict]:
        """Все пользователи (для /admin_export и старого /admin_users)"""
        cursor = self.conn.cursor()
        cursor.execute('''
            SELECT u.user_id, u.short_id, u.username, u.first_name, u.is_blocked,
                   s.is_forever, s.trial_end, s.paid_until, u.created_at
            FROM users u
            LEFT JOIN subscriptions s ON u.user_id = s.user_id
        ...
        return [{
            ...
            "first_name": r[3],
            "is_blocked": bool(r[4]),
            "is_forever": bool(r[5]) if r[5] is not None else False,
            "trial_end": r[6],
            "paid_until": r[7],
            "created_at": r[8]
        } for r in rows]
    ...
    def get_active_user_ids(self) -> List[int]:
        """user_id только с активной подпиской (для рассылки)"""
        cursor = self.conn.cursor()
        cursor.execute('''
            SELECT u.user_id
            FROM users u
            LEFT JOIN subscriptions s ON u.user_id = s.user_id
            WHERE s.is_forever = 1
               OR s.paid_until >= date('now')
               OR s.trial_end >= date('now')
        ''')
        return [r[0] for r in cursor.fetchall()]
    ...
    def get_user_activity(self, days: int = 1) -> List[Dict]:
        """Активность пользователей за N дней (для /admin_activity)"""
        cursor = self.conn.cursor()
        ...
        return [{
            ...
            "username": r[1],
            "first_name": r[2],
            "total_meals": r[3] or 0,
            "total_calories": r[4] or 0,
            "active_days": r[5] or 0,
            "first_meal": r[6],
            "last_meal": r[7]
        } for r in rows]
    ...
    def toggle_block_user(self, user_id: int) -> bool:
        """Переключает блокировку. Возвращает новое состояние (True = заблокирован)"""
        cursor = self.conn.cursor()
    ...
    def activate_forever_subscription(self, user_id: int):
        cursor = self.conn.cursor()
        cursor.execute(
            "UPDATE subscriptions SET is_active = 1, is_forever = 1, trial_end = NULL, paid_until = NULL "
            "WHERE user_id = ?",
            (user_id,)
        )
        self.conn.commit()
    ...
    def remove_subscription(self, user_id: int):
        """Удаляет подписку (обнуляет все даты)"""
        cursor = self.conn.cursor()
        cursor.execute('''
    ...
    def get_report_users(self) -> List[Dict]:
        ...
        cursor.execute('''
            ...
            WHERE report_enabled = 1
              AND (is_forever = 1 OR paid_until >= date('now') OR trial_end >= date('now'))
        ''')
        rows = cursor.fetchall()
    ...
    def add_user_product(self, user_id: int, name: str, protein: float, fat: float,
                         carbs: float, calories: float) -> bool:
        cursor = self.conn.cursor()
        try:
            cursor.execute('''
    ...
    def get_recent_meals(self, user_id: int, limit: int = 10) -> List[dict]:
        ...
        return [{
            ...
            "protein": r[1] or 0,
            "fat": r[2] or 0,
            "carbohydrates": r[3] or 0,
            "carbs": r[3] or 0,
            "calories": r[4] or 0,
            "weight_grams": r[5] or 100,
            "meal_time": r[6]
        } for r in rows]

    def get_all_user_meals(self, user_id: int, days: int = 30) -> List[dict]:
        """Все приёмы пищи за N дней (для экспорта в Excel)"""
        cursor = self.conn.cursor()
        start_date = (date.today() - timedelta(days=days)).isoformat()
        cursor.execute('''
            SELECT product_name, protein, fat, carbohydrates, calories, weight_grams, meal_time
    ...
    def get_referral_link_info(self, code: str) -> Optional[Dict]:
        cursor = self.conn.cursor()
        cursor.execute('''
            SELECT rl.code, rl.referrer_id, rl.referrer_username, rl.commission_percent, rl.bonus_months,
                   rl.created_at,
                   COALESCE(u.username, rl.referrer_username) as username,
                   COALESCE(u.first_name, rl.referrer_username) as first_name,
